...
export const useFormStore = defineStore('form', () => {
    ...
    const localStorageData: Ref<FormData | null> = ref(null)

    const loadFromLocalStorage = (): void => {
        const stored = localStorage.getItem('formData')
        localStorageData.value = stored ? JSON.parse(stored) : null
    }
    ...
    const recalculateFields = (changedField: keyof LastUpdated) => {
        ...
        switch (fieldToRecalc) {
            case 'price':
                ...
                break
            case 'qty':
                qty.value = price.value > 0 ? amount.value / price.value : 0
                break
            case 'amount':
    ...
    const saveToLocalStorage = async (): Promise<BackendResponse> => {
        ...
        if (success) {
            ...
            localStorageData.value = dataToSave
        }
